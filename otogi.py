...
class Otogi:
    ...
    def search(self, name):
        ...
        try:
            ...
            result = self.daemon_collection.find_one({"id": item_id})
            if result:
        ...
        daemons = []
        for daemon_data in self.daemon_collection.find({
            "$or": [
                {
                    att: {
                        "$regex": regex,
        ...
        if content:
            ...
            if command.startswith(("d ", "daemon ")):
                if command.startswith("d "):
                    ...
                    name = command[7:]
                self.cmd_daemon(message, name.strip())
                ...
                if command.startswith("t "):
                    ...
                    name = command[7:]
                self.cmd_trivia(message, name.strip())

    def cmd_daemon(self, message, name):
        daemons = self.search(name)
        if not daemons:
            return message.reply(f"Can't find {name} in database.")
        elif len(daemons) == 1:
            index = 0
        else:
            all_names = "\n".join([f"{i+1}. {d.name}" for i, d in enumerate(daemons)])
            if len(all_names) >= 1000:
                return message.reply("Too many results.")
            message.reply(f"Do you mean:\n{all_names}")
            try:
                msg = self.bot.wait_for("message", check=lambda m: m.author_id==message.author_id, timeout=60)
            except TimeoutError:
                return
            else:
                try:
